...
public class Blocking {
    ...
    private static void sendMatricesToWorkers(int numberOfProcessors, int n, Matrix A) {
        for (int dest = 1; dest < numberOfProcessors; dest++) {
            MPI.COMM_WORLD.Send(A.getMatrix(), dest * (n / numberOfProcessors) * n, (n / numberOfProcessors) * n, MPI.DOUBLE, dest, 0);
        }
    }

    private static void receiveMatrixFromMaster(int rank, int n, Matrix A) {
        MPI.COMM_WORLD.Recv(A.getMatrix(), rank * (n / MPI.COMM_WORLD.Size()) * n, (n / MPI.COMM_WORLD.Size()) * n, MPI.DOUBLE, 0, 0);
    }
